...
function RenderTable({ dateRange, payload }: RenderDataProps) {
	...
	const data = searchValue.trim().length > 2 ? tableData.result.data.filter((item) => (item.publisherId as string).toLowerCase().includes(searchValue.toLowerCase())) : tableData.result.data;
		...
		<div>
			<div>
				<DatePickerWithRange
					...
					onSelect={(range: any) => {
						dispatch(setDateRange({ from: String(range.from?.getTime() || ""), to: String(range.to?.getTime() || "") }));
					}}
